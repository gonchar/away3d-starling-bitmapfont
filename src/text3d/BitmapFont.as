...
package text3d
{
    ...
    public class BitmapFont
    {
        ...
        public function BitmapFont(texture:TextureMaterial=null, fontXml:XML=null)
        {
        ...
        private function parseFontXml(fontXml:XML):void
        {
            ...
            mSize = parseFloat(fontXml.info.@size);
            mLineHeight = parseFloat(fontXml.common.@lineHeight);
            mBaseline = parseFloat(fontXml.common.@base);
            ...
            for each (var charElement:XML in fontXml.chars.char)
            {
                ...
                var xAdvance:Number = parseFloat(charElement.@xadvance);
              	...
              	var regionX:Number= parseFloat(charElement.@x) + frameX;
              	var regionY:Number= parseFloat(charElement.@y) + frameY;
              	var regionWidth:Number= parseFloat(charElement.@width);
              	var regionHeight:Number= parseFloat(charElement.@height);
                
                var bitmapChar:BitmapChar = new BitmapChar(id, regionX, regionY, regionWidth, regionHeight, xOffset, yOffset, xAdvance);
        ...
        public function fillBatched(data:Vector.<Number>, indices:Vector.<uint>, width:Number, height:Number, text:String,
                                      fontSize:Number=-1,
                                      ...
                                      kerning:Boolean=true):void
        {
            ...
            for (var i:int=0; i<numChars; ++i)
            {
				...
				var x:Number = charLocation.x;
				var y:Number = charLocation.y;
				var scale:Number = charLocation.scale;
				var char:BitmapChar = charLocation.char;
				var width:Number = char.width*scale;
				var height:Number = char.height*scale;

				var u1:Number = char.x/mFontMaterial.texture.width;
				var u2:Number = (char.x+char.width)/mFontMaterial.texture.width;
				var v1:Number = char.y/mFontMaterial.texture.height;
				var v2:Number = (char.y+char.height)/mFontMaterial.texture.height;
        ...
        public function get fontMaterial():TextureMaterial { return mFontMaterial; }
    }
}

import text3d.BitmapChar;
